    ...
    <div class="card-body">
        ...
        @endif
            ...
            <table class="table align-middle mb-0">
                ...
                <tbody>
                @foreach ($penjualandetail as $penjualandetail)
                    <tr>
                        <td>{{ $penjualandetail->id }}</td>
                        <td>{{ $penjualandetail->kode_barang }}</td>
                        <td>{{ $penjualandetail->barang->nama_barang }}</td>
                        <td>{{ $penjualandetail->barang->kategori }}</td>
                        <td>{{ $penjualandetail->qty }}</td>
                        <td>{{ number_format($penjualandetail->harga) }}</td>
                        <td>{{ number_format($penjualandetail->subtotal) }}</td>
                        @if($lunas!='LUNAS')
                        <td>
                            <div class="d-flex align-items-center">

                                <form method="POST" action="{{ route('penjualandetail.destroy', ['detail_id' => $penjualandetail->id, 'penjualan_id' => $penjualandetail->penjualan_id]) }}">
                                    @csrf
                                    @method('DELETE')
